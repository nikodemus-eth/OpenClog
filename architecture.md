...
- The npm lockfile security baseline now resolves the tracked `fast-uri` audit path to `fast-uri@3.1.2`, and verification evidence remains captured through local audit and receipt lanes rather than browser-side claims.
- `@openclog/core` now models the expanded operations roadmap contract: `attentionNow`, `readinessAggregates`, `routeBudgetRegressions`, `closeoutReadiness`, `verificationReceiptDiffs`, `exportableViews`, incident templates, delivery contract previews, release-readiness gates, richer Verification Center gate metadata, and persisted incident-loop stage state.
- `@openclog/app` assembles those report fields from existing local evidence, including summary freshness, approvals, receipt history, reconnect/readiness history, route budgets, saved operator views, incident loop completion, and verification receipts.
- SQLite migrations now include durable-local prep tables for readiness snapshots, incident templates, settings history, signed bundle manifests, and native runner history while preserving Fastify as the current authority for Gateway, receipts, reporting, and incident actions.
- The web workbench renders a `Needs attention now` strip, stale-age Verification Center badges, `why blocked` drawers, redaction-aware copy actions, route-budget regression callouts, readiness aggregates, closeout readiness, delivery contract previews, release-readiness state, incident templates, persisted per-day panel state, and keyboard jumps for search, incidents, rails, Verification Center, and failed gates.

## 2026-05-17 Release 2 Trust Surfaces
- OpenClaw session recovery is now explicit architecture, not implied history: session-message backfill emits provenance metadata (`backfilled`, `importedAt`, `sourceLabel`) and repository drilldowns surface that provenance instead of blending recovered evidence with live journal flow.
- The shared operations report now carries Release 2 operator-speed fields directly from `@openclog/app`: `firstBlockedGateId`, summary-job queue depth plus oldest waiting age, saved-view stale-summary counts, and dry-run/live delivery contract parity summaries.
- The web shell uses those report fields at the existing boundary rather than inventing new policy in React: the header can jump to the first blocked Verification Center gate, collapsed rails can summarize latest verification freshness, day/archive rows can flag route-budget regressions, and saved views can distinguish stale summaries from newer-evidence drift.
- Incident-template defaults now have a workstation-local storage seam in the browser while per-day template choice remains additive on top; Fastify stays the authority for reporting, delivery policy, verification evidence, and backfill ingestion behavior.
