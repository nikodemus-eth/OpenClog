...
- Latest live route-budget proof against `http://127.0.0.1:8787` remained green after the coverage refactor and post-code-commit restart to runtime-code commit `f6d3cef`: `/api/operations/report` 396 ms / 750 ms, `/api/verification/receipts` 3 ms / 200 ms, `/api/sessions/:key` 2 ms / 300 ms, `breachCount: 0`.
